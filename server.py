...
def spiel_starten():
    ...
    def WerGewinnt(AlleSpieler, KartenMitte):
        ...
        for Spieler in AlleSpieler:
            if Spieler.get_IstDrin():
                ...
                # Hand ausgeben
                sende_alle(f'{Spieler.get_Name()} hat Hand {Spieler.get_Hand()}')
        ...
        while True: # Bis alle gesetzt haben
            if AktuellerSpieler.get_IstDrin():  # Ob der Spieler noch drin ist
                ...
                while not Valid: # Bis der Spieler eine valide Aktion gemacht hat
                    ...
                    sende_eine(client, f'aktuelle Chips: {AktuellerSpieler.get_Chips()}, Call Amount: {GesetzterBetrag}\n')
                    
                    Aktion = frage_spieler(client, f'Was möchtest du tun: ')
                    match Aktion:
                        case 'call':
                            if GesetzterBetrag > AktuellerSpieler.get_Chips():
                                sende_eine(client, 'Wenn du All in gehen willst, schreibe "all in"')
                                Valid = False
                                continue
                        ...
                        case 'raise':
                            Valid = True
                            RaiseBetrag = int(frage_spieler(client, 'Um wie viel möchtest du raisen: '))
                            if RaiseBetrag >= AktuellerSpieler.get_Chips():
                                sende_eine(client, 'Wenn du All in gehen willst, schreibe "all in"')
                                Valid = False
                                continue
                            if RaiseBetrag <= GesetzterBetrag:
                                sende_eine(client, 'Raise muss höher als Call sein')
                                Valid = False
                                continue
                        ...
                        case 'all in':
                            ...
                            Valid = True
                        case _:
                            sende_eine(client, 'Bitte nochmal(invalide Eingabe)')
            AktuellerSpieler = AktuellerSpieler.get_Nächster()
    ...
    while True:
        ...
        for Gewinner in AlleGewinner:
            sende_alle(f'{Gewinner.get_Name()} gewinnt {KartenMitte.get_Chips() // len(AlleGewinner)} Chips')
            Gewinner.add_Chips(KartenMitte.get_Chips() // len(AlleGewinner))
        
        
        for Player in AlleSpieler:
            sende_alle(f'{Player.get_Name()} hat {Player.get_Chips()} Chips')
            Player.clear_Karten()
    ...
    for Player in AlleSpieler:
        if Player.get_Chips() > Gewinner.get_Chips():
            Gewinner = Player
    sende_alle(f'{Gewinner.get_Name()} gewinnt das Spiel mit {Gewinner.get_Chips()} Chips')    
